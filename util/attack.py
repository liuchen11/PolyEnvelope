...
class PGM(Attacker):
    ...
    def __init__(self, step_size, threshold, iter_num, order = np.inf, pixel_range = None):
        '''
        >>> threshold: maximum perturbation
        >>> iter_num: the maximum number of iterations
        >>> order: norm of the attack
        >>> pixel_range: the feasible range of pixel space
        '''
        ...
        if isinstance(pixel_range, str):
            pixel_range = list(map(float, pixel_range.split(':')))
        self.pixel_range = pixel_range

        # print('Creating a PGM attack')
        # print('step_size = %.4f, threshold = %.4f, iter_num = %d, order = %f, pixel_range = %s'%(
        #    step_size, threshold, iter_num, self.order, pixel_range))
    ...
    def attack(self, model, optimizer, data_batch, label_batch, criterion = nn.CrossEntropyLoss()):
        ...
        data_batch = (data_batch + torch.randn(data_batch.shape, device = data_batch.device) * step_size).detach()
        data_batch.requires_grad_()
        ...
        for iter_idx in range(self.iter_num):
            ...
            if self.pixel_range != None:
                next_point = torch.clamp(next_point, min = self.pixel_range[0], max = self.pixel_range[1])
