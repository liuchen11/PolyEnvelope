...
def select_model(m): 
    if m.lower() in ['fc1',]:
        print('Constructing the MNIST-FC1 model')
        model = pblm.mnist_fc1()
        if torch.cuda.is_available():
            model = model.cuda()
    elif m.lower() in ['cnn',]:
        print('Constructing the MNIST-CNN model')
        model = pblm.mnist_cnn()
        if torch.cuda.is_available():
            model = model.cuda()
    else:
        raise ValueError('Unrecognized Model Type: %s' % m)
...
if __name__ == "__main__": 
    ...
    for kwarg, value in args._get_kwargs():
        print('%s --> %s' % (kwarg, value))
    ...
    for _ in range(0,args.cascade): 
        ...
        for t in range(args.epochs):
            ...
            assert args.cascade == 1, 'cascade should be 1'
